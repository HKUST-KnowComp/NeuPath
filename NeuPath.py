...
class NeuPath(nn.Module):
    def __init__(self, G, n_inp, n_hid, n_out, n_layers, n_heads, use_norm=True):
        ...
        for _ in range(n_layers):
            self.gcs.append(NeuPathLayer(n_hid, n_hid, len(G.node_dict), len(G.edge_dict), n_heads, use_norm=use_norm))
        self.out_2 = nn.Linear(n_hid, n_hid)
        self.out_1 = nn.Linear(n_hid, n_out)
        self.m = nn.ReLU()
    ...
    def forward(self, G, out_key):
        ...
        for i in range(self.n_layers):
            self.gcs[i](G, 'h', 'h')
        out = self.out_1(self.m(self.out_2(G.nodes[out_key].data['h'])))
        return out